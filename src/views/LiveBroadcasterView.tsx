...
/**
 * Gemini を使わないスタブ。
 * ここでは「そのまま返す」だけ。後で別APIに差し替えるならこの関数を書き換えればOK。
 */
async function interpretTextStub(text: string): Promise<string> {
  // 例: そのまま返す / あるいは簡単な加工
  return text;
}
...
const LiveBroadcasterView: React.FC<LiveBroadcasterViewProps> = ({ post, onEndStream }) => {
  ...
  useEffect(() => {
    ...
    const chatInterval = setInterval(async () => {
      ...
      const randomText =
        Math.random() > 0.5 ? 'This is great! 素晴らしい！' : 'すごいですね！ What is this?';
      ...
      if (isInterpreterOn) {
        const interpretation = await interpretTextStub(randomText);
  ...
  const handleSendMessage = async (e: React.FormEvent) => {
    ...
    if (isInterpreterOn) {
      const interpretation = await interpretTextStub(newMessage);
      setChatMessages((prev) =>
        prev.map((msg) => (msg.id === userMessage.id ? { ...msg, interpretation } : msg)),
      );
    }
    setNewMessage('');
  };
    ...
    <div className="fixed inset-0 bg-black text-white flex flex-col md:flex-row">
      ...
      <div className="flex-1 bg-black flex items-center justify-center relative">
        ...
        <div className="absolute top-4 left-4 flex flex-col space-y-2 z-10">
          <div className="flex items-center bg-red-600 text-white px-3 py-1 rounded-md text-sm font-bold">
            LIVE
          </div>
        ...
        <div className="absolute top-4 right-4 z-10">
          <button
            onClick={onEndStream}
            className="flex items-center space-x-2 px-4 py-2 bg-red-600 text-white font-bold rounded-full hover:bg-red-700 transition-colors"
          >
            <StopIcon />
        ...
        <div className="p-3 border-b border-gray-700 flex justify-between items-center">
          ...
          <div className="flex items-center space-x-2">
            ...
            <label className="relative inline-flex items-center cursor-pointer">
              <input
                type="checkbox"
                checked={isInterpreterOn}
                onChange={() => setInterpreterOn(!isInterpreterOn)}
          ...
          {chatMessages.map((msg) => (
            <div key={msg.id} className="flex items-start space-x-2 mb-3 text-sm">
              ...
              <div>
                ...
                {msg.interpretation && (
                  <p className="text-blue-400 text-xs italic mt-1 break-words">
                    {msg.interpretation}
                  </p>
                )}
              </div>
